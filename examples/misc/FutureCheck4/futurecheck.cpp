...
/*
 * Code size check for Future API StaticFutureManager (moveable Future).
 * Wiring:
 * - Arduino UNO
...
#include <fastarduino/single_future.h>
...
using FUTURE_MANAGER = SingleFutureManager;
...
int main()
{
...

	{
		...
		ASSERT_STATUS(out, READY, future1);
		ASSERT_STATUS(out, INVALID, future2);
		ASSERT_VALUE(out, VAL, future1);
	}
}
